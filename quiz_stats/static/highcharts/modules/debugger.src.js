    ...
    _registerModule(_modules, 'error-messages.js', [_modules['Core/Globals.js']], function (H) {
        ...
        H.errorMessages = {
            ...
            "12": {
                ...
                "text": "<h1>Highcharts expects point configuration to be numbers or arrays in turbo mode</h1><p>This error occurs if the <highcharts>series.data</highcharts> option contains object configurations and the number of points exceeds the turboThreshold. It can be fixed by either setting <highcharts>turboThreshold</highcharts> to a higher value, or changing the point configurations to numbers or arrays.</p><p>In boost mode, turbo mode is always on, which means only array of numbers or two dimensional arrays are allowed.</p><p>See <a href=\"https://api.highcharts.com/highcharts#plotOptions.series.turboThreshold\">plotOptions.series.turboThreshold</a></p>"
            },
            "13": {
            ...
            "23": {
                ...
                "text": "<h1>Unsupported color format used for color interpolation</h1><p>Highcharts supports three color formats primarily: hex (<highcharts>#FFFFFF</highcharts>), rgb (<highcharts>rgba(255,255,255)</highcharts>) and rgba (<highcharts>rgba(255,255,255,1)</highcharts>). If any other format, like 3-digit colors (<highcharts>#FFF</highcharts>), named colors (<highcharts>white</highcharts>) or gradient structures are used in for example a heatmap, Highcharts will fail to interpolate and will instead use the end-color with no interpolation applied.</p><p>We've chosen to preserve this limitation in order to keep the weight of the implementation at a minimum.</p>"
            },
            "24": {
                "title": "Cannot run Point.update on a grouped point",
                "text": "<h1>Cannot run Point.update on a grouped point</h1><p>Running <highcharts>Point.update</highcharts> in Highstock when a point is grouped by data grouping is not supported.</p><p>This is not supported because when data grouping is enabled, there won't be any references to the raw points, which is required by the <highcharts>Point.update</highcharts> function.</p>"
            },
            "25": {
